...
export async function getGlobalLeaderboard(
  ...
  try {
    ...
    if (tier) {
      entries = await prisma.$queryRaw<LeaderboardEntry[]>`
        SELECT
      ...
      totalResult = await prisma.$queryRaw<[{ count: bigint }]>`
        SELECT COUNT(*) as count FROM users WHERE tier = ${tier}
      `;
    } else {
      entries = await prisma.$queryRaw<LeaderboardEntry[]>`
        SELECT
 ...
 */
export async function getUserRankInfo(
...
async function calculateTierProgress(
  ...
  const tierThresholds: Record<
    string,
    { min: number; next: string | null; nextThreshold: number }
  > = {
  ...
  const progressPercent = nextTier
    ? Math.round(
        ((totalPoints - tier.min) / (tier.nextThreshold - tier.min)) * 100,
      )
    : 100;
